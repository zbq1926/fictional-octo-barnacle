...
export default {
    async fetch(request, env, ctx) {
        try {
            ...
            myID = env.UUID || env.uuid || env.PASSWORD || env.pswd || myID;
            if (env.KEY || env.TOKEN || (myID && !isValidUUID(myID))) {
                动态UUID = env.KEY || env.TOKEN || myID;
                有效时间 = Number(env.TIME) || 有效时间;
                更新时间 = Number(env.UPTIME) || 更新时间;
                const myIDs = await 生成动态UUID(动态UUID);
                myID = myIDs[0];
                myIDLow = myIDs[1];
            } else 动态UUID = myID;
            ...
            const fakemyIDMD5 = await 双重哈希(`${myID}${timestamp}`);
            const fakemyID = [
                fakemyIDMD5.slice(0, 8),
                fakemyIDMD5.slice(8, 12),
                fakemyIDMD5.slice(12, 16),
                fakemyIDMD5.slice(16, 20),
                fakemyIDMD5.slice(20)
            ].join('-');

            const fakeHostName = `${fakemyIDMD5.slice(6, 9)}.${fakemyIDMD5.slice(13, 19)}`;

            奸商IP = env.奸商IP || env.奸商IP || 奸商IP;
            奸商IPs = await 整理(奸商IP);
            奸商IP = 奸商IPs[Math.floor(Math.random() * 奸商IPs.length)];
            ...
            if (socks5Address) {
                try {
                    ...
                    反代IP = env.R奸商IP || 'false';
                    enableSocks = true;
                } catch (err) {
                    let e = err;
                    console.log(e.toString());
                    反代IP = env.R奸商IP || !奸商IP ? 'true' : 'false';
                    enableSocks = false;
                }
            } else {
                反代IP = env.R奸商IP || !奸商IP ? 'true' : 'false';
            ...
            if (!upgradeHeader || upgradeHeader !== 'websocket') {
                ...
                if (url.searchParams.has('奸商IP')) {
                    path = `/奸商IP=${url.searchParams.get('奸商IP')}`;
                    反代IP = 'false';
                ...
                if (路径 == '/') {
                    ...
                    const fakeConfig = await 生成配置信息(myID, request.headers.get('Host'), sub, 'CF-Workers-SUB', 反代IP, url, fakemyID, fakeHostName, env);
                    return new Response(`${fakeConfig}`, { status: 200 });
                } else if ((url.pathname == `/${动态UUID}/config.json` || 路径 == `/${myID}/config.json`) && url.searchParams.get('token') === await 双重哈希(fakemyID + UA)) {
                    return await config_Json(myID, request.headers.get('Host'), sub, UA, 反代IP, url, fakemyID, fakeHostName, env);
                } else if (url.pathname == `/${动态UUID}/edit` || 路径 == `/${myID}/edit`) {
                ...
                if (url.searchParams.has('奸商IP')) {
                    奸商IP = url.searchParams.get('奸商IP');
                    enableSocks = false;
                } else if (new RegExp('/奸商IP=', 'i').test(url.pathname)) {
                    奸商IP = url.pathname.toLowerCase().split('/奸商IP=')[1];
                    enableSocks = false;
                } else if (new RegExp('/奸商IP.', 'i').test(url.pathname)) {
                    奸商IP = `奸商IP.${url.pathname.toLowerCase().split("/奸商IP.")[1]}`;
                    enableSocks = false;
                } else if (new RegExp('/pyip=', 'i').test(url.pathname)) {
                    奸商IP = url.pathname.toLowerCase().split('/pyip=')[1];
                    enableSocks = false;
                }

                return await 韦莱斯OverWSHandler(request);
            }
        } catch (err) {
            let e = err;
    ...
    readableWebSocketStream.pipeTo(new WritableStream({
        async write(chunk, controller) {
            ...
            const {
                ...
                addressRemote = '',
                rawDataIndex,
                韦莱斯Version = new Uint8Array([0, 0]),
                isUDP,
            } = process韦莱斯Header(chunk, myID);
            // 设置地址和端口信息，用于日志
            ...
            // 构建 韦莱斯 响应头部
            const 韦莱斯ResponseHeader = new Uint8Array([韦莱斯Version[0], 0]);
            // 获取实际的客户端数据
            const rawClientData = chunk.slice(rawDataIndex);

            if (isDns) {
                // 如果是 DNS 查询，调用 DNS 处理函数
                //return handleDNSQuery(rawClientData, webSocket, 韦莱斯ResponseHeader, log);
                const { write } = await handleUDPOutBound(webSocket, 韦莱斯ResponseHeader, log);
            ...
            if (!banHosts.includes(addressRemote)) {
                ...
                handleTCPOutBound(remoteSocketWapper, addressType, addressRemote, portRemote, rawClientData, webSocket, 韦莱斯ResponseHeader, log);
            } else {
...
async function handleTCPOutBound(remoteSocket, addressType, addressRemote, portRemote, rawClientData, webSocket, 韦莱斯ResponseHeader, log,) {
    ...
    async function nat64() {
        if (!useSocks) {
            const nat64奸商IP = `[${await resolveToIPv6(addressRemote)}]`;
            log(`NAT64 代理连接到 ${nat64奸商IP}:443`);
            tcpSocket = await connectAndWrite(nat64奸商IP, 443);
        }
        ...
        remoteSocketToWS(tcpSocket, webSocket, 韦莱斯ResponseHeader, null, log);
    }
    ...
    async function retry() {
        if (enableSocks) {
            ...
            if (!奸商IP || 奸商IP == '') {
                奸商IP = atob('UFJPWFlJUC50cDEuMDkwMjI3Lnh5eg==');
            } else if (奸商IP.includes(']:')) {
                portRemote = 奸商IP.split(']:')[1] || portRemote;
                奸商IP = 奸商IP.split(']:')[0] + "]" || 奸商IP;
            } else if (奸商IP.split(':').length === 2) {
                portRemote = 奸商IP.split(':')[1] || portRemote;
                奸商IP = 奸商IP.split(':')[0] || 奸商IP;
            }
            if (奸商IP.includes('.tp')) portRemote = 奸商IP.split('.tp')[1].split('.')[0] || portRemote;
            tcpSocket = await connectAndWrite(奸商IP.toLowerCase() || addressRemote, portRemote);
        }
        ...
        */
        ...
        remoteSocketToWS(tcpSocket, webSocket, 韦莱斯ResponseHeader, nat64, log);
    }
 ...
 * @param {string} myID 用于验证的用户 ID
 * @returns {Object} 解析结果，包括是否有错误、错误信息、远程地址信息等
 */
function process韦莱斯Header(韦莱斯Buffer, myID) {
    // 检查数据长度是否足够（至少需要 24 字节）
    if (韦莱斯Buffer.byteLength < 24) {
        return {
            hasError: true,
    ...
    // 解析 韦莱斯 协议版本（第一个字节）
    const version = new Uint8Array(韦莱斯Buffer.slice(0, 1));

    let isValidUser = false;
    let isUDP = false;

    // 验证用户 ID（接下来的 16 个字节）
    function ismyIDValid(myID, myIDLow, buffer) {
        const myIDArray = new Uint8Array(buffer.slice(1, 17));
        const myIDString = stringify(myIDArray);
        return myIDString === myID || myIDString === myIDLow;
    }

    // 使用函数验证
    isValidUser = ismyIDValid(myID, myIDLow, 韦莱斯Buffer);

    // 如果用户 ID 无效，返回错误
    if (!isValidUser) {
        return {
            hasError: true,
            message: `invalid user ${(new Uint8Array(韦莱斯Buffer.slice(1, 17)))}`,
        };
    }

    // 获取附加选项的长度（第 17 个字节）
    const optLength = new Uint8Array(韦莱斯Buffer.slice(17, 18))[0];
    // 暂时跳过附加选项

    // 解析命令（紧跟在选项之后的 1 个字节）
    // 0x01: TCP, 0x02: UDP, 0x03: MUX（多路复用）
    const command = new Uint8Array(
        韦莱斯Buffer.slice(18 + optLength, 18 + optLength + 1)
    )[0];
    ...
    const portBuffer = 韦莱斯Buffer.slice(portIndex, portIndex + 2);
    ...
    const addressBuffer = new Uint8Array(
        韦莱斯Buffer.slice(addressIndex, addressIndex + 1)
    );
    ...
    switch (addressType) {
        case 1:
            ...
            addressValue = new Uint8Array(
                韦莱斯Buffer.slice(addressValueIndex, addressValueIndex + addressLength)
            ).join('.');
            break;
        case 2:
            // 域名
            ...
            addressLength = new Uint8Array(
                韦莱斯Buffer.slice(addressValueIndex, addressValueIndex + 1)
            )[0];
            addressValueIndex += 1;
            // 解码域名
...
async function remoteSocketToWS(remoteSocket, webSocket, 韦莱斯ResponseHeader, retry, log) {
    ...
    let 韦莱斯Header = 韦莱斯ResponseHeader;
        ...
        .pipeTo(
            new WritableStream({
                ...
                async write(chunk, controller) {
                    ...
                    if (韦莱斯Header) {
                        // 如果有 韦莱斯 响应头部，将其与第一个数据块一起发送
                        webSocket.send(await new Blob([韦莱斯Header, chunk]).arrayBuffer());
                        韦莱斯Header = null; // 清空头部，之后不再发送
                    } else {
                        // 直接发送数据块
 ...
 * @param {ArrayBuffer} 韦莱斯ResponseHeader 
 * @param {(string)=> void} log 
 */
async function handleUDPOutBound(webSocket, 韦莱斯ResponseHeader, log) {

    let is韦莱斯HeaderSent = false;
        ...
        async write(chunk) {
            ...
            if (webSocket.readyState === WS_READY_STATE_OPEN) {
                ...
                if (is韦莱斯HeaderSent) {
                    ...
                    webSocket.send(await new Blob([韦莱斯ResponseHeader, udpSizeBuffer, dnsQueryResult]).arrayBuffer());
                    is韦莱斯HeaderSent = true;
                }
            }
        }
    })).catch((error) => {
 ...
 */
async function handleDNSQuery(udpChunk, webSocket, 韦莱斯ResponseHeader, log) {
    ...
    try {
        ...
        let 韦莱斯Header = 韦莱斯ResponseHeader; // 保存 韦莱斯 响应头部，用于后续发送
        ...
        await tcpSocket.readable.pipeTo(new WritableStream({
            async write(chunk) {
                if (webSocket.readyState === WS_READY_STATE_OPEN) {
                    if (韦莱斯Header) {
                        // 如果有 韦莱斯 头部，则将其与 DNS 响应数据合并后发送
                        webSocket.send(await new Blob([韦莱斯Header, chunk]).arrayBuffer());
                        韦莱斯Header = null; // 头部只发送一次，之后置为 null
                    } else {
                        // 否则直接发送 DNS 响应数据
                        webSocket.send(chunk);
                    }
                ...
                if (lines[0].split(',').length > 3) {
                    ...
                    for (let i = 1; i < lines.length; i++) {
                        ...
                        if (columns) {
                            ...
                            if (api[index].includes('奸商IP=true')) 奸商IPPool.push(`${columns}:${测速端口}`);
                        }
                    }
                } else {
                    // 验证当前apiUrl是否带有'奸商IP=true'
                    if (api[index].includes('奸商IP=true')) {
                        // 如果URL带有'奸商IP=true'，则将内容添加到奸商IPPool
                        奸商IPPool = 奸商IPPool.concat((await 整理(content)).map(item => {
        ...
        try {
            ...
            for (let i = 1; i < lines.length; i++) {
                ...
                if (columns[tlsIndex].toUpperCase() === tls && parseFloat(columns[speedIndex]) > DLS) {
                    ...
                    if (csvUrl.includes('奸商IP=true') && columns[tlsIndex].toUpperCase() == 'true' && !httpsPorts.includes(port)) {
                        // 如果URL带有'奸商IP=true'，则将内容添加到奸商IPPool
                        奸商IPPool.push(`${ipAddress}:${port}`);
                    }
                }
            }
        } catch (error) {
...
function 生成本地订阅(host, UUID, noTLS, newAddressesapi, newAddressescsv, newAddressesnotlsapi, newAddressesnotlscsv) {
    ...
    const responseBody = uniqueAddresses.map(address => {
        ...
        let 最终路径 = path;
        let 节点备注 = '';
        const matching奸商IP = 奸商IPPool.find(奸商IP => 奸商IP.includes(address));
        if (matching奸商IP) 最终路径 = `/奸商IP=${matching奸商IP}`;
        /*
        ...
        try {
            ...
            if (ipSource === 'as13335') {
                ...
                for (const line of allLines) {
                    const parsedIP = parse奸商IPLine(line, targetPort);
                    if (parsedIP) {
...
131.0.72.0/22`;
    ...
    // 新增：解析反代IP行的函数
    function parse奸商IPLine(line, targetPort) {
        try {
            // 移除首尾空格
            line = line.trim();
            if (!line) return null;
    ...
    <div class="test-controls">
        <div class="port-selector">
            ...
            <select id="ip-source-select">
                ...
                <option value="奸商IP">反代IP列表</option>
            </select>
...
async function config_Json(myID, hostName, sub, UA, 反代IP, _url, fakemyID, fakeHostName, env) {
    ...
    else if (奸商IP && 奸商IP != '') CF访问方法 = "奸商IP";
    else if (反代IP == 'true') CF访问方法 = "auto";
    
    let 域名地址 = hostName;
    let 端口 = 443;
    let 传输层安全 = ['tls', true];
    if (hostName.includes('.workers.dev')) {
        域名地址 = fakemyID + '.' + hostName + '.cf.090227.xyz';
        端口 = 80;
        传输层安全 = ['', false];
    }
    ...
    const config = {
        ...
        config: {
            ...
            KEY: (uuid != myID) ? {
                DynamicUUID: true,
                TOKEN: uuid || null,
                UUID: myID.toLowerCase() || null,
                UUIDLow: myIDLow || null,
                TIME: 有效时间 || null,
                UPTIME: 更新时间 || null,
                fakemyID: fakemyID || null,
            } : {
                DynamicUUID: false,
                UUID: myID.toLowerCase() || null,
                fakemyID: fakemyID || null,
        ...
        link: {
            v2: `${atob(啥啥啥_写的这是啥啊)}://${myID.toLowerCase()}@${域名地址}:${端口}\u003f\u0065\u006e\u0063\u0072\u0079` + 'p' + `${atob('dGlvbj0=')}none\u0026\u0073\u0065\u0063\u0075\u0072\u0069\u0074\u0079\u003d${传输层安全[0]}&sni=${域名地址}&fp=randomized&type=ws&host=${hostName}&path=${encodeURIComponent(path) + allowInsecure}&fragment=${encodeURIComponent('1,40-60,30-50,tlshello')}#${encodeURIComponent(FileName)}`,
            clash: `- {name: ${FileName}, server: ${域名地址}, port: ${端口}, type: ${atob(啥啥啥_写的这是啥啊)}, uuid: ${myID.toLowerCase()}, tls: ${传输层安全[1]}, alpn: [h3], udp: false, sni: ${域名地址}, tfo: false, skip-cert-verify: ${SCV}, servername: ${hostName}, client-fingerprint: randomized, network: ws, ws-opts: {path: '${path}', headers: {Host: ${hostName}}}}`,
        },
...
async function 生成配置信息(myID, hostName, sub, UA, 反代IP, _url, fakemyID, fakeHostName, env) {
    ...
    if (userAgent.includes('mozilla') && !subParams.some(_searchParams => _url.searchParams.has(_searchParams))) {
        const token = await 双重哈希(fakemyID + UA);
        ...
        let url = `${subProtocol}://${sub}/sub?host=${fakeHostName}&uuid=${fakemyID}&奸商IP=${反代IP}&path=${encodeURIComponent(path)}&${atob('ZWRnZXR1bm5lbD1jbWxpdQ==')}`;
        let isBase64 = true;
        ...
        if (!sub || sub == "") {
            ...
            url = `https://${hostName}/${fakemyID + _url.search}`;
        ...
        try {
            ...
            if ((!sub || sub == "") && isBase64 == true) {
                content = await 生成本地订阅(fakeHostName, fakemyID, noTLS, newAddressesapi, newAddressescsv, newAddressesnotlsapi, newAddressesnotlscsv);
            } else {
                const response = await fetch(url, {
                    headers: {
            ...
            if (_url.pathname == `/${fakemyID}`) return content;

            return 恢复伪装信息(content, myID, hostName, fakemyID, fakeHostName, isBase64);
...
<body>
    ...
    <div id="advancedModal" class="modal">
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <div class="setting-item">
                    <label class="setting-label">
                        <input type="checkbox" id="奸商IPEnabled" onchange="updateProxySettings('奸商IP')">
                        <span class="checkmark"></span>
                        🌐 奸商IP
                    </label>
                    <input type="text" id="奸商IPInput" placeholder="奸商IP.cmliussss.net:443" class="setting-input">
                </div>
    ...
    <script>
        ...
        function buildSubscriptionUrl(host, uuid, suffix) {
            ...
            // 处理代理参数（互斥）
            if (settings.奸商IPEnabled && settings.奸商IPValue) {
                params.push('奸商IP=' + encodeURIComponent(settings.奸商IPValue));
        ...
        function renderConfigInfo() {
            ...
            if (config.KEY.DynamicUUID) {
                ...
                configItems = [
                    ...
                    { label: 'FKID', value: config.KEY.fakemyID },
        ...
        function renderProxyInfo() {
            ...
            const proxy = configData.奸商IP;
            let items = [];

            if (proxy.Request奸商IP === 'true') {
                ...
                if (cf2cdn === '奸商IP') {
                    items.push({ label: 'CloudflareCDN访问模式', value: '奸商IP' });
                    if (proxy.List.PROXY_IP && proxy.List.PROXY_IP.length > 0) {
                        items.push({ label: '奸商IP列表', value: proxy.List.PROXY_IP.join('<br>') });
                    }
        ...
        function loadAdvancedSettings() {
            ...
            document.getElementById('奸商IPEnabled').checked = settings.奸商IPEnabled;
            document.getElementById('奸商IPInput').value = settings.奸商IPValue;
            document.getElementById('奸商IPInput').disabled = !settings.奸商IPEnabled;
        ...
        function saveAdvancedSettings() {
            ...
            const settings = {
                ...
                奸商IPEnabled: document.getElementById('奸商IPEnabled').checked,
                奸商IPValue: document.getElementById('奸商IPInput').value,
        ...
        function updateProxySettings(type) {
            ...
            if (enabled) {
                // 取消其他代理选项的勾选
                const proxyTypes = ['奸商IP', 'socks5', 'http'];
